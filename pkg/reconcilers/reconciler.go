...
import (
	...
	ciliumv2ac "github.com/cilium/cilium/pkg/k8s/client/clientset/versioned/typed/cilium.io/v2"
...
func reconcileDeployment(
	...
	if err := reconcilePodDisruptionBudget(
		...
		PodDisruptionBudgetMode{
			Value: slices.Ternary(replicas > 1, uint32(1), uint32(0)),
...
func reconcileNetworkPolicy(
	...
	return tracing.WithSpan1(ctx, "networkPolicy", "reconcileNetworkPolicy",
		func(ctx context.Context, span trace.Span) error {
			...
			var ciliumNetworkPolicyInterface ciliumv2ac.CiliumNetworkPolicyInterface
...
func reconcileKubernetesNetworkPolicy(
	ctx context.Context,
	networkPolicyInterface v1.NetworkPolicyInterface,
	name string,
	namespace string,
	ownerReference *metav1ac.OwnerReferenceApplyConfiguration,
	labels map[string]string,
...
func reconcileCiliumNetworkPolicy(
	ctx context.Context,
	ciliumNetworkPolicyInterface ciliumv2ac.CiliumNetworkPolicyInterface,
	name string,
	namespace string,
	ownerReference *metav1ac.OwnerReferenceApplyConfiguration,
	labels map[string]string,
	...
	return tracing.WithSpan1(ctx, "networkPolicy", "reconcileCiliumNetworkPolicy",
		func(ctx context.Context, span trace.Span) error {
							...
							ToEndpoints: []api.EndpointSelector{
								{
									LabelSelector: &ciliummetav1.LabelSelector{
										MatchLabels: map[string]ciliummetav1.MatchLabelsValue{
											"k8s-app":                  "kube-dns",
											ciliumio.PodNamespaceLabel: "kube-system",
										},
									},
								},
							},
			...
			if ownerReference != nil {
				ciliumNetworkPolicyApplyConfiguration.ObjectMeta.OwnerReferences = []metav1.OwnerReference{
					{
			...
			if invalidConditions := slices.Filter(ciliumNetworkPolicy.Status.Conditions,
				...
				return fmt.Errorf("cilium network policy %s/%s is invalid: %s", namespace, name,
...
func reconcilePodDisruptionBudget(
	...
	value := intstr.FromInt32(int32(mode.Value))
	switch mode.Type {
	...
	default:
		return fmt.Errorf(
			"pod disruption budget %s/%s: %w",
			namespace,
			name,
			errors.New("unknown pod disruption budget type"),
		)
	}
